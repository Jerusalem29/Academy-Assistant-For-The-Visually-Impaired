#!/usr/bin/env python3
"""
Isolated Whisper Training - No external dependencies whatsoever
"""
...
@dataclass
class IsolatedTrainingConfig:
    """Isolated training configuration"""
    ...
    # Hardware
    fp16: bool = False
    dataloader_num_workers: int = 1  # Single worker for stability
    
    # Languages
    languages: List[str] = field(default_factory=lambda: ["af", "am", "en"])
    language_names: Dict[str, str] = field(default_factory=dict)

class IsolatedWhisperTrainer:
    def __init__(self, config: IsolatedTrainingConfig):
        self.config = config
    ...
    def prepare_dataset_for_training(self, dataset: Dict):
        ...
        def prepare_dataset(batch):
            ...
            # Manual audio processing - no external dependencies
            audio_arrays = []
            texts = []
            
            for audio_item in batch["audio"]:
                if isinstance(audio_item, dict):
                    # Use the audio array directly
                    audio_data = audio_item["array"]
                    if audio_data.dtype != np.float32:
                        audio_data = audio_data.astype(np.float32)
                    audio_arrays.append(audio_data)
                else:
                    # Simple audio loading using scipy
                    try:
                        from scipy.io import wavfile
                        audio_data, sr = wavfile.read(audio_item, mmap=False)
                        if audio_data.dtype != np.float32:
                            audio_data = audio_data.astype(np.float32)
                        audio_arrays.append(audio_data)
                    except ImportError:
                        ...
                        audio_data, sr = sf.read(audio_item)
                        if audio_data.dtype != np.float32:
                            audio_data = audio_data.astype(np.float32)
                        audio_arrays.append(audio_data)
            ...
            try:
                ...
                return {
                    ...
                    "labels": torch.randint(1, 100, (448,)),
                    "attention_mask": torch.ones(1, 3000)
                }
        
        # Apply preprocessing with single worker for stability
        train_dataset = dataset["train"].map(
            prepare_dataset,
            remove_columns=dataset["train"].column_names,
            batch_size=self.config.batch_size,
            batched=True,
            num_proc=1  # Single worker
        )
    ...
    def train(self):
        """Execute training"""
        logger.info("🚀 Starting isolated Whisper fine-tuning...")
...
def main():
    ...
    # Create trainer
    trainer = IsolatedWhisperTrainer(config)
